...
private:
  ...
  static fk::vector<P> exact_solution_dim2(fk::vector<P> const x, P const t = 0)
  {
    ...
    fk::vector<P> fx(x.size());
    std::transform(x.begin(), x.end(), fx.begin(),
                   [](P const &x) { return cos(2.0 * PI * x / 3.0); });
    return fx;
  }

  static P exact_time(P const time) { return sin(2.0 * time); }
  ...
  static P source_0_time(P const time) { return 2.0 * cos(2.0 * time); }
  ...
  static P source_2_time(P const time) { return -PI * sin(2.0 * time); }
  ...
  static fk::vector<P> source_3_dim2(fk::vector<P> const x, P const t = 0)
  {
    ...
    fk::vector<P> fx(x.size());
    std::transform(x.begin(), x.end(), fx.begin(),
                   [](P const &x) { return sin(2.0 * PI * x / 3.0); });
    return fx;
  }

  static P source_3_time(P const time)
  {
    return -2.0 / 3.0 * PI * sin(2.0 * time);
  }
